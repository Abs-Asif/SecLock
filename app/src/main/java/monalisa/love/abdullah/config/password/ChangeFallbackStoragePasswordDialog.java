...
public final class ChangeFallbackStoragePasswordDialog extends PasswordDialog {

    public ChangeFallbackStoragePasswordDialog(Activity activity, LockStore lockStore, Runnable onSuccess) {
        super(activity, lockStore, onSuccess, R.string.fallback_storage_password_change_title,
    ...
    protected void build() {
        ...
        positiveBtn.setOnClickListener(v -> {
            ...
            if (lockStore.fallbackStoragePasswordMatch(currentPassword)) {
                if (lockStore.setFallbackStoragePassword(newPassword)) {
                    dismiss();
                    onSuccess.run();
                }
            } else {
        ...
        neutralBtn.setOnClickListener(v -> {
            lockStore.removeFallbackStoragePassword();
            onSuccess.run();
            dismiss();
        });
    }
