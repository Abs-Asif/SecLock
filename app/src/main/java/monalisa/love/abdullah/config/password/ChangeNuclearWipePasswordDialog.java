...
public final class ChangeNuclearWipePasswordDialog extends PasswordDialog {

    public ChangeNuclearWipePasswordDialog(Activity activity, LockStore lockStore, Runnable onSuccess) {
        super(activity, lockStore, onSuccess, R.string.nuclear_wipe_password_change_title,
    ...
    protected void build() {
        ...
        positiveBtn.setOnClickListener(v -> {
            ...
            if (lockStore.nuclearWipePasswordMatch(currentPassword)) {
                if (lockStore.setNuclearWipePassword(newPassword)) {
                    dismiss();
                    onSuccess.run();
                }
            } else {
        ...
        neutralBtn.setOnClickListener(v -> {
            lockStore.removeNuclearWipePassword();
            onSuccess.run();
            dismiss();
        });
    }
